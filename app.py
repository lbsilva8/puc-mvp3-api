...
def add_serie(form: SerieSchema):
    """Adiciona uma nova serie/filme à base de dados
...
def get_series():
    """ Faz a busca de todas as series/filmes cadastradas
    
    Retorna uma representação da listagem de serie/filme.
    """
...
def get_anime():
    ...
    logger.debug(f"Coletando series")
    session = Session()
    categoria = session.query(Serie).filter(Serie.categoria == 'anime')

    if not categoria:
        return {"series": []}, 200
    else:
        logger.debug(f"%d series econtradas")
        print(categoria)
        return apresenta_series(categoria), 200


@app.get('/serie', tags=[serie_tag],
         responses={"200": SerieViewSchema, "404": ErrorSchema})
def get_serie(query: SerieBuscaIdSchema):
    """ Faz a busca da serie/filme a partir de um id
    
    Retorna uma representação da serie/filme.
    """
...
def del_serie(query: SerieBuscaNomeSchema):
    """Deleta uma serie/filme a partir do nome
